...
def post_stock_reconciliation(docname):
    try:
        ...
        if stock_reconciliation.custom_voucher_no_server1:
        ...
        server = "http://102.216.33.196/api/resource/Stock%20Reconciliation"
        ...
        if posted_reconciliation:
            frappe.db.set_value("Stock Reconciliation", docname, "custom_voucher_no_server1", posted_reconciliation)
...
def check_and_update_item(api_keys, item_dict):
    ...
    server = f"http://102.216.33.196/api/resource/Item/{item_dict['item_code']}"
...
def create_item(api_keys, item_dict):
    ...
    server = "http://102.216.33.196/api/resource/Item"
